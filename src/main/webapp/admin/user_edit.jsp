...
<head>
    ...
    <script type="text/javascript">
        ...
        %>
        $.get("${pageContext.request.contextPath}/user?method=userEditPage&id=<%=id%>", function (data) {
            if (data.code === 0) {
                console.log(data)
                let user = data.data;
                $("#userid").val(user.id);
                ...
                $("#avatar").attr("src", "../static/LightYearAdmin/images/users/"+user.avatar);
                $("#avatarImage").val(user.avatar);
                if (user.type === "0")
                    $("#typeOne").prop("checked", true);
                else
                    $("#typeTwo").prop("checked", true);

                if (user.status === "0")
                    $("#statusTwo").prop("checked", true);
                else
        ...
        $(function () {
            ...
            $(".ajax-post").click(function () {
                ...
                $.post(
                    "${pageContext.request.contextPath}/user?method=userEdit",
                    $("#postForm").serialize(),
                    function(data) {
                        //返回值为 0 代表登录成功
                        if (data.code === 0) {
